...
const dbConfig = {
  ...
  user: 'uaxghndj',
  password: 'LieeMjI99OqUAYjnw5844TE6hwv9QtE5',
  database: 'uaxghndj',
  port: 5432
}
      ...
      on('task', {
        ...
        deleteStudent(studentEmail) {
          return new Promise(function (resolve, reject) {
            ...
            // vai substituir o $1 pelo valor do studentEmail
            pool.query(query, [studentEmail], function (error, result) {
              if (error) {
                reject({ error: error })
              }
        ...
        resetStudent(student) {
          return new Promise(function (resolve, reject) {
            ...
            // Primeiro deleta e depois insere 
            const query = `
            WITH add AS (
            ...
            const values = [student.name, student.email, student.age, student.weight,
            student.feet_tall]
